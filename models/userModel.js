...
userSchema.statics.signup = async function (
    ...
    const user = await this.create({ email, password: hash, sex, bio, joined, dpUrl, firstName, userName, lastName, notifications, balance, likes, followers })  //so we write an instance to create an email and password and we set the pwrd to the hash const so that it can combine the pwrd and salt . so we store everything the user const so we can use it has a custom method

...
const getPostsPaginated = async (req, res) => {
    try {
        ...
        const allPosts = await Posts.find().skip(skip).limit(pageSize);
...
const getAllPosts = async (req, res) => {
    try {
        ...
        const allPosts = await Posts.find().skip(parseInt(skip)).limit(ITEMS_PER_PAGE);
...
const getUserPosts = async (req, res) => {
    ...
    try {
        ...
        if (!userPosts || userPosts.length === 0) {
            return res.status(404).json({ error: 'You have not made any post' });
        }
